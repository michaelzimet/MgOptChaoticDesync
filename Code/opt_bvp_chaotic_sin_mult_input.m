...
for k=1:11
   ...
   [t,x] = ode45('func_opt_chaotic_sin_mult_input',[0:0.01:t1],[theta0,lambda0],options);
...
err = Htheta^2;
   ...
   [tp,xp] = ode45('func_opt_chaotic_sin_mult_input',[0:0.01:t1],[theta0,lambda0+dlambda],options);
   [tm,xm] = ode45('func_opt_chaotic_sin_mult_input',[0:0.01:t1],[theta0,lambda0-dlambda],options);
...
topt = t;
%exp for u
uopt = min(M,max(-M,((B*Zp_sin(theta)-lambda.*Z_sin(theta))/2)));
...
for i = 1:num_inputs
   % integrate until theta1 = 2*pi
    [tsoln_new,theta_soln_new] = ode45(@func_integrate_sin_mult_input,[0:0.01:20],[theta10,theta20],opt);
...
p = polyfit(tsoln,log(theta2_soln-theta1_soln), 1);
slope = p(1);
