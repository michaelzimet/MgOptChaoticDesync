...
%for M in M_range
j=12;
opt_bvp_chaotic_sin_mult_input;
close all
in(j, :) = uopt;
...
lyap_calc(j) = sum(Zp_sin(theta).*uopt*0.01)/(2*pi);
...
AddLetters2Plots(figure(2), {'A', 'B','C','D', 'E', 'F'},  'HShift', -0.08, 'VShift', -0.015)

saveas(figure(2), '../Optimal Control Figs/Fig2.eps', 'epsc')
